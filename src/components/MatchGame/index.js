...
const newRandomNumber = Math.ceil(Math.random() * imagesList.length - 1)
const newRandomList = imagesList[newRandomNumber]

class MatchGame extends Component {
  state = {
    score: 0,
    activeTabId: tabsList[0].tabId,
    activeImageId: newRandomList.id,
    activeImageUrl: newRandomList.imageUrl,
    isStateSet: true,
    count: 60,
  }
  ...
  onStartGame = () => {
    const startRandomNumber = Math.ceil(Math.random() * imagesList.length - 1)
    const startRandomList = imagesList[startRandomNumber]
    this.setState({
      score: 0,
      activeTabId: tabsList[0].tabId,
      activeImageId: startRandomList.id,
      activeImageUrl: startRandomList.imageUrl,
      isStateSet: true,
      count: 60,
    })
  ...
  tick = () => {
    const {count, isStateSet} = this.state
    if (count === 1) {
      this.setState({isStateSet: !isStateSet})
  ...
  onChangeImage = newId => {
    ...
    const newList = imagesList[randomNumber]

    if (newId === activeImageId) {
      const {id, imageUrl} = newList

      this.setState({activeImageId: id, activeImageUrl: imageUrl})
      this.setState(prevState => ({
        score: prevState.score + 1,
      }))
    } else if (newId !== activeImageId) {
      this.setState({isStateSet: false})
  ...
  render() {
    const {activeTabId, activeImageUrl, score, count, isStateSet} = this.state
    const newList = this.onFilterNewList(imagesList)
    return (
